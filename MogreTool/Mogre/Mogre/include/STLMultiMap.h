...
#define INC_DECLARE_STLMULTIMAP_NONCONST( CLASS_NAME, MK, MV, NK, NV )					\
ref class CLASS_NAME : Collections::Generic::IEnumerable<Collections::Generic::KeyValuePair<MK, MV>>	\
{																					\
public:																				\
	typedef std::multimap<NK,NV> Container;												\
...
public:																				\
	...
	void Erase(MK key);																\
																					\
	Iterator^ Insert(MK key, MV value);												\
																					\
	...
	Iterator^ UpperBound(MK key);													\
																					\
	int CountValues(MK key);														\
																					\
	Mogre::Pair<Iterator^, Iterator^> EqualRange(MK key);							\
																					\
	Collections::Generic::List<MV>^ GetValues(MK key);								\
};
...
#define INC_DECLARE_STLMULTIMAP_CONST( CLASS_NAME, MK, MV, NK, NV )					\
...
public:																				\
	...
	int CountValues(MK key);														\
																					\
	Mogre::Pair<ConstIterator^, ConstIterator^> EqualRange(MK key);					\
																					\
	Collections::Generic::List<MV>^ GetValues(MK key);								\
};
...
#define INC_DECLARE_STLMULTIMAP_EMPTY_NONCONST( CLASS_NAME, MK, MV, NK, NV )		\
...
#define CPP_DECLARE_STLMULTIMAP_NONCONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )				\
	...
	{																				\
		_native->erase(ToNative<MK,NK>( key ));										\
																					...
																					\
	PREFIX##CLASS_NAME::Iterator^ PREFIX##CLASS_NAME::Insert(MK key, MV value)		\
	{																				\
		return _native->insert( std::pair<NK,NV>( ToNative<MK,NK>( key ), ToNative<MV,NV>( value ) ) );	\
	}																				\
																					\
	...
	{																				\
		return _native->upper_bound(ToNative<MK,NK>( key ));						\
																					...
																					\
	int PREFIX##CLASS_NAME::CountValues(MK key)										\
	{																				\
		return _native->count(ToNative<MK,NK>( key ));								\
	}																				\
																					\
	Mogre::Pair<PREFIX##CLASS_NAME::Iterator^, PREFIX##CLASS_NAME::Iterator^> PREFIX##CLASS_NAME::EqualRange(MK key)				\
	{																				\
		std::pair<Iter,Iter> range = _native->equal_range(ToNative<MK,NK>( key ));	\
		return Mogre::Pair<PREFIX##CLASS_NAME::Iterator^, PREFIX##CLASS_NAME::Iterator^>(range.first, range.second);	\
	}																				\
																					\
	Collections::Generic::List<MV>^ PREFIX##CLASS_NAME::GetValues(MK key)			\
	{																				\
		Collections::Generic::List<MV>^ values = gcnew Collections::Generic::List<MV>();	\
																					\
		std::pair< Iter, Iter > pair =												\
            _native->equal_range( ToNative<MK,NK>( key ) );							\
																					\
		for (Iter i = pair.first; i != pair.second; ++i)							\
        {																			\
			values->Add( ToManaged<MV,NV>( i->second ) );							\
        }																			\
																					\
		return values;																\
	}
...
#define CPP_DECLARE_STLMULTIMAP_CONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )						\
	...
	{																				\
		return _native->upper_bound(ToNative<MK,NK>( key ));						\
																					...
																					\
	int PREFIX##Const_##CLASS_NAME::CountValues(MK key)								\
	{																				\
		return _native->count(ToNative<MK,NK>( key ));								\
	}																				\
																					\
	Mogre::Pair<PREFIX##Const_##CLASS_NAME::ConstIterator^, PREFIX##Const_##CLASS_NAME::ConstIterator^> PREFIX##Const_##CLASS_NAME::EqualRange(MK key)	\
	{																				\
		std::pair<Iter,Iter> range = _native->equal_range(ToNative<MK,NK>( key ));	\
		return Mogre::Pair<PREFIX##Const_##CLASS_NAME::ConstIterator^, PREFIX##Const_##CLASS_NAME::ConstIterator^>(range.first, range.second);	\
	}																				\
																					\
	Collections::Generic::List<MV>^ PREFIX##Const_##CLASS_NAME::GetValues(MK key)	\
	{																				\
		Collections::Generic::List<MV>^ values = gcnew Collections::Generic::List<MV>();	\
																					\
		std::pair< Iter, Iter > pair =												\
...
#define CPP_DECLARE_STLMULTIMAP_EMPTY_NONCONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )				\
																					\
	PREFIX##Const_##CLASS_NAME^ PREFIX##CLASS_NAME::ReadOnlyInstance::get()							\
		{																			\
			if (_readOnly == (Object^)nullptr)										\
...
#define INC_DECLARE_STLMULTIMAP( CLASS_NAME, MK, MV, NK, NV, PUBLIC, PRIVATE )			\
ref class Const_##CLASS_NAME;														\
PUBLIC INC_DECLARE_STLMULTIMAP_NONCONST( CLASS_NAME, MK, MV, NK, NV )					\
PUBLIC INC_DECLARE_STLMULTIMAP_CONST( CLASS_NAME, MK, MV, NK, NV )


#define INC_DECLARE_STLMULTIMAP_READONLY( CLASS_NAME, MK, MV, NK, NV, PUBLIC, PRIVATE )	\
ref class Const_##CLASS_NAME;														\
PRIVATE INC_DECLARE_STLMULTIMAP_EMPTY_NONCONST( CLASS_NAME, MK, MV, NK, NV )				\
PUBLIC INC_DECLARE_STLMULTIMAP_CONST( CLASS_NAME, MK, MV, NK, NV )



#define CPP_DECLARE_STLMULTIMAP( PREFIX, CLASS_NAME, MK, MV, NK, NV )						\
CPP_DECLARE_STLMULTIMAP_NONCONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )						\
CPP_DECLARE_STLMULTIMAP_CONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )


#define CPP_DECLARE_STLMULTIMAP_READONLY( PREFIX, CLASS_NAME, MK, MV, NK, NV )				\
CPP_DECLARE_STLMULTIMAP_EMPTY_NONCONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )					\
CPP_DECLARE_STLMULTIMAP_CONST( PREFIX, CLASS_NAME, MK, MV, NK, NV )
